...
### 1. Feed / Home (`(tabs)/index.tsx`) — la vista principal
- Lista de cards de eventos cercanos, ordenados por fecha/distancia por defecto.
- Cada card: foto, título, fecha/hora, distancia, chip de categoría, avatar-stack de "quiénes van".
- Barra de filtros arriba (fecha, categoría, distancia) — chips horizontales scrolleables.
- Botón flotante o entrada al mapa como vista alternativa (toggle feed/mapa, no dos pantallas separadas).
- Pull-to-refresh.
- Estado vacío: "No hay eventos cerca todavía — sé el primero en publicar uno."

### 2. Detalle de evento
- Header con foto/portada del evento.
- Título, fecha, ubicación (con mini-mapa o dirección), descripción, categoría.
- Avatar-stack + conteo de "van" / "les interesa".
- Botón principal: "Voy" / "Me interesa" (dos estados distintos).
- Si eres el creador: acciones de editar/cancelar en vez del botón de asistencia.
